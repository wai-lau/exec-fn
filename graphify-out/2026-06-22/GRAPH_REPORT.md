# Graph Report - exec-fn  (2026-06-22)

## Corpus Check
- 110 files · ~4,593,346 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 927 nodes · 1520 edges · 57 communities (46 shown, 11 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 200 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0ae81685`
...
- `guest_login_page()` --calls--> `_index_pages()`  [INFERRED]
  api/routes_views.py → api/pages.py
- `guest_login_page()` --calls--> `_tmpl()`  [INFERRED]
  api/routes_views.py → api/pages.py
...
Cohesion: 0.12
Nodes (14): api_context_patch(), api_rd_patch(), api_todos_add(), api_todos_delete(), _atomic_write_json(), _flag_triage(), _log_entries_for_patch(), _minutes_late() (+6 more)
...
Cohesion: 0.19
Nodes (16): _build_nav(), _index_pages(), Page composition: nav builder, index-shell variants, template loader.  Pure rend, Return (no_form, bare) variants of /app/static/index.html, re-read on change., _render_page(), _tmpl(), color_page(), debug_page() (+8 more)
...
Cohesion: 0.07
Nodes (34): _lifespan(), _no_cache_static(), Request, FastAPI entry point: app, lifespan, middleware, 401 redirects, wiring.  Routes l, unauthorized_handler(), The three top-level APIRouters, shared by every route module.  Defined here (not, api_debug_logs(), color_usage() (+26 more)
...
Cohesion: 0.19
Nodes (12): Scoped bearer auth for /api/exec/say. EXEC_SAY_KEY only grants message-queueing,, require_auth(), require_guest_auth(), require_say_auth(), api_gcal_auth(), api_gcal_import_cards(), api_gamesave_delete(), api_gamesave_get() (+4 more)
...
Cohesion: 0.14
Nodes (16): guest_login(), guest_login_alias(), guest_login_page(), _landing_html(), login(), login_page(), HTML page routes + the read-only data GETs that back them.  Public landing/login, Public landing page: non-admin sections only, as a centered vertical     column (+8 more)
...
Cohesion: 0.06
Nodes (54): addStreamDiv(), _caretOffset(), _caretToEnd(), focusInput(), _focusNow(), _inputBar, _inputCursor, _msgInput (+46 more)
...
Cohesion: 0.10
Nodes (19): _active_nudge_block(), _dedupe_context(), _focused_nudge_card(), get_chat(), Most-recently-nudged card with an active nudge loop., _parse_json(), Extract and parse the first JSON object or array from a string., build_morning() (+11 more)
...
Cohesion: 0.11
Nodes (25): addMsg(), _caretOffset(), _imgCache, _inputBar, _inputCursor, _linkifyRules(), messages, _msgInput (+17 more)
...
Cohesion: 0.18
Nodes (11): classify_card(), parse_date_natural(), Card-creation LLM helpers: classify a new card's category + importance, and pars, api_parse_date(), api_prophecies_patch(), api_rd_classify(), api_rd_recalc(), Rebuild a card's breakdown on demand (the dialog's 'recalculate' button),     in (+3 more)
...
## Knowledge Gaps
- **137 isolated node(s):** `RULES — READ FIRST`, `System overview`, `File map`, `Terminology`, `Pages` (+132 more)
...
- **Why does `build_morning()` connect `Exec Chat Tools` to `Community 36`, `Community 46`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `api_morning()` connect `Community 46` to `Community 0`, `Exec Chat Tools`, `Community 34`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `FastAPI` connect `MTG Rules Assistant` to `Community 0`, `Community 38`, `Community 34`, `Community 46`?**
