# Graph Report - exec-fn  (2026-06-10)

## Corpus Check
- 82 files · ~4,578,480 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 613 nodes · 1055 edges · 49 communities (36 shown, 13 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 113 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e4bb5cf2`
...
Cohesion: 0.08
Nodes (38): _apply_reminder_flag(), _apply_schedule(), _apply_size_time(), _nudge_resched_blocked(), Exec-chat scheduling: load/save wrapper around scheduler.schedule_to_day., Due dates are protected: an active-nudge card can't be deferred without the, _tool_create_card(), _tool_delete_card() (+30 more)
...
Cohesion: 0.09
Nodes (38): _dedupe_context(), _tool_reschedule(), _advance_recurrence(), _next_recurrence(), _now_et(), _parse_json(), date, Advance a date by one recurrence step. None for unknown type. (+30 more)
...
Cohesion: 0.12
Nodes (14): _active_nudge_block(), _build_chat_system_prompt(), _focused_nudge_card(), get_chat(), Most-recently-nudged card with an active nudge loop., get_rd_log(), api_prophecies_log(), api_rd_log() (+6 more)
...
Cohesion: 0.13
Nodes (18): _handle_tool(), _load_json(), api_assemble_plan(), api_context(), api_context_patch(), api_directives_get(), api_plan_get(), api_profile() (+10 more)
...
Cohesion: 0.15
Nodes (14): classify_card(), parse_date_natural(), api_parse_date(), api_rd_classify(), api_rd_recalc(), login(), login_page(), _no_cache_static() (+6 more)
...
Cohesion: 0.14
Nodes (14): append_monitor_comment(), api_nudge_tick(), _build_graph(), _fire_nudge(), _nudge_tick(), push_to_monitor(), Manual one-shot tick of the nudge loop (the in-process loop runs this     automa, Generate + deliver one nudge (or stall re-peel). Reloads rd around the     LLM c (+6 more)
...
Cohesion: 0.24
Nodes (11): _entry_is_significant(), monitor_flush(), Fire monitor immediately if significant activity exists since last comment., Trailing debounce: each call resets the 60s timer., _run_monitor(), _schedule_monitor(), _build_context(), _entry_line() (+3 more)

### Community 46 - "Community 46"
Cohesion: 0.33
Nodes (6): api_debug_logs(), _build_nav(), graph_page(), nightfall_page(), build_nightfall_html(), Path
...
## Knowledge Gaps
- **94 isolated node(s):** `RULES — READ FIRST`, `System overview`, `File map`, `Terminology`, `Pages` (+89 more)
...
- **Why does `api_rd_patch()` connect `Morning Pipeline` to `Exec Chat Tools`, `Community 45`, `Community 37`, `Community 39`?**
