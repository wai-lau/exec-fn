# Graph Report - exec-fn  (2026-06-24)

## Corpus Check
- 123 files · ~4,612,870 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1165 nodes · 2043 edges · 64 communities (51 shown, 13 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 246 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6b610908`
...
8. `_now_et()` - 15 edges
9. `_save_rd()` - 15 edges
10. `Numerology Across Pips and Courts` - 15 edges
...
- `_rl_check()` --calls--> `HTTPException`  [INFERRED]
  api/tarot/routes.py → api/main.py
- `api_debug_logs()` --calls--> `Path`  [INFERRED]
  api/routes_views.py → api/tarot/routes.py
- `graph_page()` --calls--> `Path`  [INFERRED]
  api/routes_views.py → api/tarot/routes.py
...
Cohesion: 0.05
Nodes (72): _active_nudge_block(), _build_chat_system_prompt(), _focused_nudge_card(), Most-recently-nudged card with an active nudge loop., _apply_reminder_flag(), _apply_schedule(), _apply_size_time(), _nudge_resched_blocked() (+64 more)
...
Cohesion: 0.13
Nodes (21): _dedupe_context(), create_gcal_event(), _dedup_key(), fetch_calendar_events(), _fetch_gcal_raw_full(), _fetch_ics_events(), fetch_omens(), _gcal_creds() (+13 more)
...
Cohesion: 0.23
Nodes (13): $(), applyVolume(), checkHealth(), loadVoices(), PARAM_IDS, params(), player, reflectBackend() (+5 more)
...
## Knowledge Gaps
- **146 isolated node(s):** `RULES — READ FIRST`, `System overview`, `File map`, `Terminology`, `Pages` (+141 more)
...
- **Why does `FastAPI` connect `Community 5` to `Community 0`, `MTG Rules Assistant`, `Tarot Reader Engine`?**
