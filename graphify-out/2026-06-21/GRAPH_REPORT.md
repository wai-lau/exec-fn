# Graph Report - exec-fn  (2026-06-21)

## Corpus Check
- 106 files · ~4,587,206 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 855 nodes · 1404 edges · 53 communities (41 shown, 12 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 194 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `027edd95`
...
Cohesion: 0.27
Nodes (11): _build_context(), _entry_is_significant(), _entry_line(), flush_monitor(), generate_encouragement(), _is_commentable(), Trailing debounce: each call resets the 60s timer., Fire the monitor now if significant activity exists since the last     comment ( (+3 more)
...
Cohesion: 0.22
Nodes (9): api_gcal_auth(), api_gcal_import_cards(), api_rd_recalc(), Rebuild a card's breakdown on demand (the dialog's 'recalculate' button),     in, api_gamesave_delete(), api_gamesave_get(), api_gamesave_post(), serve_data() (+1 more)
...
Cohesion: 0.06
Nodes (51): _array_re(), _drop_graph_book_nodes(), Serve-time scrubbing of graphify's /graph page.  graph.html is regenerated whole, Match `[const |var |let ]NAME = [ ... ];` on its own line. Group 1 is the     as, Return the parsed array for `name`, or None if absent/unparseable., Find the `name` array, json-parse it, apply `transform`, splice the result     b, Blank the label+title of every _GRAPH_REDACT_IDS node to "[redacted]" in     gra, Remove every RAW_NODES entry whose source_file is under the tarot book     dir, (+43 more)

### Community 35 - "Community 35"
Cohesion: 0.24
Nodes (15): addToggles(), buildPhysicsColumn(), clusterSpan(), focusRandomCluster(), go(), hideOrphans(), initTour(), makeControls() (+7 more)
...
- **Why does `build_morning()` connect `Exec Chat Tools` to `Community 16`, `Community 36`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `api_morning()` connect `Community 16` to `Community 0`, `Community 8`, `Exec Chat Tools`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `HTTPException` connect `Community 8` to `Community 34`, `Tarot Reader Engine`, `Community 37`, `Community 38`, `MTG Rules Assistant`, `Community 16`?**
