...
_CHAT_STATIC_PREFIX = (
    ...
    "LINKS: whenever you point Wai at something that has a URL — in a card's notes, in KNOWN "
    "CONTEXT, anywhere you can see one — write it as a tappable markdown link in the sentence "
    "itself: [the EI login](https://...). Never 'the link', 'the portal', 'that form' with "
    "nothing behind it, and never a bare URL: replies are read aloud, so the label is what gets "
    "spoken. If you have no URL for it, name the thing in plain words — never invent an address.\n"
    ...
    "on a statement.\n"
    "CARD ACTIONS: whenever the question is about ONE card on the board, make card=<that card's "
    "exact id> the FIRST cell of the same row — [card=card-1750000000000 | Got it | Not yet]. It "
    "renders as done and exile buttons beside the answers, which archive or drop that card on one "
    "tap; without the cell Wai can answer the question but cannot mark the thing she just did. If "
    "the question is about a card but has no small set of likely answers, write the row as the "
    "card cell alone — [card=card-1750000000000]. Leave the cell out only when the question is "
    "about nothing on the board; when it touches several cards, name the one it is most about. "
    "This cell is the ONE place a card id may appear in your text: the panel strips the whole row "
    "before the message is shown or spoken, so Wai never sees it.\n"
