...
- `_drop_graph_prefixed_nodes`: cut whole source trees (the Pollack tarot
  reference book, vendored libs, the nightfall game's own React source).
...
- `_drop_graph_orphan_nodes`: cut the nodes no surviving edge touches.
...
# Whole source trees that are indexed but are not this codebase's structure.
# One tuple rather than three near-identical passes, because the argument is the
# same every time: graphify indexes what is on disk, and some of what is on disk
# is somebody else's code or a reference text.
#   api/tarot/book/            the Pollack tarot reference — 100+ concept nodes
#                              (card meanings, numerology) that drown the code.
#                              The tarot ENGINE stays; only the book is cut.
#   web/vendor/                vendored third-party libs, one node per mangled
#                              minified name: Kv(), _f(), Le(), ...
#   .../nightfall-src/         the nightfall game's React/TS source — 1054 nodes,
#                              a quarter of the whole graph and the biggest
...
def _drop_graph_prefixed_nodes(page: str) -> str:
    """Remove every RAW_NODES entry whose source_file sits under one of
    `_GRAPH_DROP_PREFIXES`, along with its dangling edges/legend/hyperedges.
    No-op if RAW_NODES is absent/unparseable or nothing matched."""
    nodes = _read_array(page, "RAW_NODES")
    if not nodes:
        return page
    drop_ids = {
        n.get("id")
        for n in nodes
        if str(n.get("source_file") or "").startswith(_GRAPH_DROP_PREFIXES)
    }
...
def _drop_graph_orphan_nodes(page: str) -> str:
    ...
    which meant they were still parsed, still built into the DataSet, and still
    walked on every redraw. Dropping them at serve time takes them out of the
    payload entirely, and the header counts then describe what is actually drawn.

    Must run AFTER `_drop_graph_inferred_edges` — an edge dropped later would
    orphan a node this pass had already kept. No-op if RAW_NODES is absent."""
    nodes = _read_array(page, "RAW_NODES")
    if not nodes:
        return page
    live = set()
    for e in _read_array(page, "RAW_EDGES") or []:
        live.add(e.get("from"))
        live.add(e.get("to"))
    return _prune_graph_nodes(
        page, {n.get("id") for n in nodes if n.get("id") not in live}
    )
