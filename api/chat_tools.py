...
def _tool_decompose_task(input_: dict) -> dict:
    ...
    card = _find_card(_load_rd(), input_.get("id", ""))
    if not card:
        return {"error": f"Card not found: {input_.get('id')}"}
    if card.get("is_reminder") or card.get("is_book"):
        return {"error": "Reminders and books can't be decomposed."}
    # LLM call outside the lock — holding it for seconds would stall every
    # other rd.json writer. Reload after, like nudge_loop._fire_nudge.
