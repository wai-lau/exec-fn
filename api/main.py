...
_GRAPH_NAV_STYLE = (
    "<style>"
    # graph.html body is a flex row; make the nav the left column via order:-1
    "body{display:flex!important;height:100vh!important;overflow:hidden!important;margin:0!important;}"
    ".exec-nav{position:static!important;flex-direction:column;order:-1;"
    "flex:0 0 64px;width:64px;height:100vh;box-sizing:border-box;"
    "justify-content:space-evenly;padding:12px 0;overflow-y:auto;"
    "border-top:none!important;border-right:1px solid rgba(var(--green-rgb),0.12)!important;}"
    ".exec-nav a{flex:0 0 auto!important;max-width:none!important;}"
    "</style>"
)
...
async def graph_page():
    ...
    # /graphify). Inject chrome.css + a vertical-left nav override + the nav itself.
    ...
    html = p.read_text()
    html = html.replace("</head>", _CHROME_LINK + _GRAPH_NAV_STYLE + "</head>", 1)
    html = html.replace("</body>", _build_nav("graph") + "</body>", 1)
