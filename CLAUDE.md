...
exec-fn/
  ...
  web/                    # static frontend (index.html, fonts, card-dialog.js, images)
    ...
    tarot-persona.js      # /tarot reader persona picker. ONE chooser: `chooseScreen(onPick)` renders the **"who reads for you?"** gate (one `.reader-pick` .spread-btn per persona) inside #terminal before a fresh reading — the pick is the user gesture that selects the persona, unlocks audio (`tarotVoice.unlock()`), and fires onPick. Once you pick you are COMMITTED for the session (no mid-reading switch — the cycle button was removed). Cast fetched from GET /api/tarot/personas (`ready` promise; `list()`); selection persists in localStorage `tarot.persona` (default `reader`). `tarotPersona.current()` (seeded synchronously from localStorage) rides with each chat turn; the server restyles every non-default persona (see api/tarot/restyle.py). select() points tarot-voice.js at the persona's TTS voice/backend/gain. Loaded AFTER tarot-voice.js (needs tarotVoice) + BEFORE tarot-stream.js. The OPENING is gated by this screen + eager-generated for EVERY persona in parallel (tarot-chat.js `beginOpening` fires `prefetchOpening(id)` per persona; the pick replays the chosen buffered response via `streamResponse(null, prefetched)` and aborts the rest — instant reveal, no LLM wait). Degraded single-persona (endpoint failed) → `openingNoChoice` falls back to the original eager-hold-for-gesture opening
  ...
  api/
    ...
    tarot/
      routes.py           # /api/tarot/{spreads,cards,personas,draw,chat}; in-process IP rate-limit on /chat; _is_opening() detects the fresh-start turn (drives the persona opener)
      agent.py            # streaming tool-use loop with text + tool_call SSE events. stream_chat(persona_brief, persona_opener): default streams reader text live; a persona HOLDS each round's text + re-voices it via restyle.py (haiku) before emit, tool_use blocks pass through in order. Helpers: _drive_stream/_emit_restyled/_emit_tools/_sse
      restyle.py          # persona 2nd-pass: restyle_stream(text, brief, opener) — haiku re-voices the reader's canonical text in the persona voice. _PRESERVE locks card substance (bold names, position/orientation/meaning, next-position invite, single-question shape; INVENT-NOTHING rule so a bare question never confabulates cards) while freeing voice; with `opener` set it instead delivers the persona's opener (which may be LITERAL lines or a beat-SPEC the model fills + varies) on the opening turn. Falls back to canonical text on error
      personas.py         # reader persona registry: PERSONAS{id,name,voice_id,backend,gain,brief,opener?}. `reader` (af_nicole) = default, brief None = single-pass/no restyle. `glados` (voice glados/piper) = GLaDOS voice brief + opener SPEC (a 3-beat instruction — "Oh, it's you." / a varied subconscious-or-weight quip / a what's-on-your-mind question — not literal text, so the quip varies each visit). get_persona_brief/get_persona_opener/public_list (brief stripped — server-only)
...
| `/hosaka` | **Protected** TTS page (full auth, not guest). SPEAK UI streaming audio from a home GPU box (Kokoro/Chatterbox) via a same-origin WS reverse-proxy (`routes_tts.py`); browser only ever talks same-origin so the session cookie carries auth on the WS handshake. |
...
| `/tarot` | Tarot reading: spread (top, fixed-height) + Pollack-voiced reader chat (bottom); guest auth; per-browser state in `localStorage` (no server persistence). **Reader narration** (`tarot-voice.js`, AUDIBLE by default; works for full `session` AND `guest_session`): the reader's turn is spoken via hosaka (voice `af_nicole`) and the typewriter paces to the actual audio clock — `tarot-chat.js` holds the text until audio starts (reader "draws breath" behind a blinking cursor), then reveals chars by `charWeight`-shaped schedule normalized to the measured audio duration (preserves punctuation pauses, no drift, self-corrects off `player.elapsed()`). The ♪ button in `#spread-controls` is a MUTE (drops volume to 0; narration still streams + paces the typewriter). Audio fail / not-yet-unlocked → the guessed-pace typewriter. Optional **ambient music** (`tarot-music.js`, ♫ toggle below the reset button): a looping background track at a 15% bed (baked into the file — iOS ignores el.volume), no ducking, streamed lazily from `web/tarot-ambient.m4a`; starts + fades in (4s) on the first tap, from a random point. Optional **reader persona** (`tarot-persona.js`, chosen on the "who reads for you?" screen — committed for the session, no mid-reading switch; localStorage `tarot.persona`, default `reader`): picks which character voices the reading. The default reader is the canonical Pollack/Gibson voice (single-pass, untouched); any other persona keeps the same card SUBSTANCE but re-voices the delivery through a 2nd haiku pass server-side (`api/tarot/restyle.py`) and narrates in the persona's TTS voice. A fresh reading opens behind a **"who reads for you?"** screen (every persona's opening eager-generated in parallel; the pick reveals instantly). Currently `reader` + `glados` (GLaDOS — deadpan/clinical, own fixed opener) |
...
| GET | `/api/tarot/personas` | Reader persona menu `{personas:[{id,name,voice_id,backend,gain}]}` (voice briefs are server-only, stripped). Drives the /tarot persona picker. |
...
| POST | `/api/tarot/chat` | Body `{messages, spread: {type, revealed, face_down_positions, significator?}, persona?}` → SSE stream. Server told only about revealed cards; face-down identities never leave the browser. `persona` (default `reader`) selects a delivery voice — non-default personas get a 2nd-pass restyle (see restyle.py). In-process per-IP rate limit (20 req / 60s). |
...
**Personas (delivery skin, not a reading change).** The reader engine (phases, tools, significator, card meanings) is voice-agnostic and runs unchanged for every persona. A non-default `persona` adds a 2nd pass: the reader generates the canonical turn (opus, with tools), then `restyle.py` re-voices its text via haiku — card substance locked (which card, position, orientation, meaning, bold names, next-position invite), voice + attitude free. The default `reader` skips the 2nd pass entirely (no latency cost; the eager-opening + audio-pacing path is untouched). A persona may carry a fixed `opener` delivered (varied) on the fresh-start turn instead of the reader's atmospheric image. Adding a persona = one entry in `tarot/personas.py` (id/name/voice_id/backend/gain/brief/opener) — no engine change.

**Opening = "who reads for you?" gate (eager-both).** A fresh reading no longer auto-starts. `tarot-chat.js` `beginOpening()` pushes the opening marker, then EAGER-GENERATES every persona's opening in parallel (`prefetchOpening(id)` → a started `/api/tarot/chat` fetch per persona, server streaming into buffers, body unread) and shows the persona screen (`tarotPersona.chooseScreen`). The pick is the audio-unlock gesture: it replays the chosen persona's buffered response (`streamResponse(null, prefetched)`) instantly and aborts the others. `streamResponse(holdForGesture, prefetched)` uses a passed-in `Promise<Response>` instead of issuing its own fetch when `prefetched` is set. Trade-off: N personas = N opening generations per fresh reading (1 narrated). The browser progression tests (`tests/test_tarot_progression.py`) click a `.reader-pick` to start the turn.
