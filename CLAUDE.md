...
exec-fn/
  ...
  api/
    ...
    data/                 # persistent volume (./api/data → /app/data)
      ...
      EXEC_*.rmdoc        # timestamped EXEC rMdoc pulls from reMarkable
      EXEC_YYYYMMDD_baseline.rmdoc
      WAI_*.rmdoc         # WAI rMdocs (original rM filename)
      WAI_*.pdf           # PDF plans
```
...
| rMdoc | Any reMarkable notebook file (`.rmdoc`) — EXEC rMdocs, WAI rMdocs, etc. |
...
| Method | Path | What |
|--------|------|------|
| POST | `/api/morning` | Build PDF plan + upload to rM + pull baseline EXEC rMdoc |
...
1. `build_pdf.build(WAI_<ts>.pdf)` — build PDF plan from plan.json
2. `rmapi put --force WAI_<ts>.pdf` — upload PDF plan → becomes rMdoc plan on device
...
1. `pull_wai()` — pull latest WAI rMdoc from rM (skip if local mtime ≥ rM ModifiedClient)
