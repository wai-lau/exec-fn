...
| `/debug` | Profile notes + activity log viewer + saved tarot readings. Styled as **"dark recruiter"** via the shared **document theme** — chrome.css defines `--doc-*` tokens (mirroring `/recruiter`'s `html.cv-dark` palette; brand hues only) + reusable `.doc-card` / `.doc-h2` / `.doc-chip` classes (centered dark card, green glow, terminal-green ink, ruled uppercase section headers, tag chips). The reading/dashboard pages (`/debug`, `/emet`, `/color`, `/mtg`, `/tarot` chat, `/security`) apply those classes in their template + style only page-specific content on the `--doc-*` tokens; the dense boards (`/rd`, `/hq`) stay as-is. Recruiter remains the source of truth for the palette. |
