...
- `prep_time`: of `estimated_time`, the **prep** minutes — the hands-on lead-up steps that get decomposed (the part Wai stalls on). The remainder (`estimated_time - prep_time` = `work`) is the **event**: an atomic external occurrence you simply attend (class, concert, appointment, show, meeting) and never split. A self-directed task has `work = 0` (all prep, no event block). Auto-filled at creation (exec chat `create_card`, `_card_brief` budgets prep-vs-event in decompose). Editable in the card dialog as two fields on the **date line** (`cd-prep` + `cd-dur`, labelled prep + **duration** — the field UI-labelled "event" was renamed "duration"); the **recalculate** button stays in the breakdown row and rebuilds the graph to that split. null for reminders.
