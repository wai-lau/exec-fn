...
| `/emet` | **Protected** placeholder static page. Raw `templates/emet.html` served as-is (no chrome/nav inject) via `emet_page()` in routes_views.py. Lives in `api/templates/` (not the public `/app/static` mount) so it stays auth-gated. Nav label = `אמת` (emet, "truth"), icon = `golem-stone.png` (nightfall GolemStone sprite), next to graph. |
