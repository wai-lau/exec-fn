...
exec-fn/
  ...
  web/                    # static frontend (index.html, fonts, card-dialog.js, images)
    ...
    tarot-voice.js        # /tarot reader narration: a voice toggle (#voice-btn in #spread-controls, localStorage `tarot.voice`, default OFF) + a HosakaAudio player speaking the reader's text with voice af_nicole (kokoro). tarotVoice.ready() = on AND `player.gestureUnlocked()` (a user gesture has unlocked the player — NOT the live `isUnlocked()`/ctx.state, which false-negatives on Windows Chrome and silently dropped the voice). The toggle-tap unlocks iOS audio; the auto-triggered opening turn has no gesture, so it stays silent until first interaction. tarot-chat's drainAudio has a watchdog: if voice is attempted but the audio clock never advances (a device that genuinely can't play), it hands off to the guessed-pace typewriter so text never freezes. Persisted-on across a reload (`tarot.voice`==1 but no gesture yet) would otherwise read on-but-locked forever; `armPersistedUnlock()` arms a one-shot `pointerdown`/`keydown` (capture, in-gesture, no setTimeout) that unlocks the player on the user's first interaction, so it narrates without a re-toggle. tarotVoice.speak(md) strips markdown, fires TTS, returns a controller {elapsed(),duration(),ended,ok} the typewriter polls. NOTE: /ws/hosaka requires the full `session` cookie, so audio only works for logged-in Wai — guests (guest_session) fall back to the silent guessed-pace typewriter
