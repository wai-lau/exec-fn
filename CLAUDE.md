...
exec-fn/
  ...
  api/
    ...
    tarot/
      ...
      restyle.py          # persona 2nd-pass: restyle_stream(text, brief, opener) — haiku re-voices the reader's canonical text in the persona voice. _PRESERVE locks card substance (bold names, position/orientation/meaning, next-position invite, single-question shape; INVENT-NOTHING rule so a bare question never confabulates cards) while freeing voice; with `opener` set it instead delivers the persona's fixed opener (varied) on the opening turn. Falls back to canonical text on error
      personas.py         # reader persona registry: PERSONAS{id,name,voice_id,backend,gain,brief,opener?}. `reader` (af_nicole) = default, brief None = single-pass/no restyle. `glados` (voice glados/chatterbox) = GLaDOS voice brief + fixed opener. get_persona_brief/get_persona_opener/public_list (brief stripped — server-only)
...
| `/tarot` | Tarot reading: spread (top, fixed-height) + Pollack-voiced reader chat (bottom); guest auth; per-browser state in `localStorage` (no server persistence). Optional **reader narration** (`tarot-voice.js`, ♪ toggle in `#spread-controls`, default OFF; works for full `session` AND `guest_session`): the reader's turn is spoken via hosaka (voice `af_nicole`) and the typewriter paces to the actual audio clock — `tarot-chat.js` holds the text until audio starts (reader "draws breath" behind a blinking cursor), then reveals chars by `charWeight`-shaped schedule normalized to the measured audio duration (preserves punctuation pauses, no drift, self-corrects off `player.elapsed()`). Voice off / audio fail / not-yet-unlocked → the original guessed-pace typewriter. Optional **ambient music** (`tarot-music.js`, ♫ toggle below the reset button): a looping background track at a 15% bed (baked into the file — iOS ignores el.volume), no ducking, streamed lazily from `web/tarot-ambient.m4a`; starts + fades in (4s) on the first tap, from a random point. Optional **reader persona** (`tarot-persona.js`, cycle button in `#spread-controls`, localStorage `tarot.persona`, default `reader`): picks which character voices the reading. The default reader is the canonical Pollack/Gibson voice (single-pass, untouched); any other persona keeps the same card SUBSTANCE but re-voices the delivery through a 2nd haiku pass server-side (`api/tarot/restyle.py`) and narrates in the persona's TTS voice. A fresh reading opens behind a **"who reads for you?"** screen (every persona's opening eager-generated in parallel; the pick reveals instantly). Currently `reader` + `glados` (GLaDOS — deadpan/clinical, own fixed opener) |
