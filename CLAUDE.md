...
exec-fn/
  ...
  scripts/                # version-controlled git hooks: pre-commit (lint + page smoke tests + docs reminder) + install-hooks.sh (symlinks into .git/hooks) + lint-colors.py (palette discipline: no new colors/alphas; allowed (color,alpha) pairs DERIVED from /api/color/usage, frozen in palette-baseline.json; `--update` regenerates it) + palette-baseline.json (the frozen per-color alpha allowlist)
  tests/                  # pytest suites vs the live container: test_smoke.py (HTTP over every route by auth tier) + test_exec_voice.py (HTTP: which pages wire the GLaDOS voice scripts — planning=panel, other protected=listener, /tarot+/hosaka excluded) + conftest.py. WebKit/playwright browser suites (marked `browser`, run as dedicated pre-commit steps when their layer changes): test_tarot_progression.py (a reading always progresses) + test_exec_voice_browser.py (Exec speaks its turns in the glados voice — boundary mocked, no TTS box). Run: .venv/bin/pytest tests/ -q (add `-m browser` + `playwright install webkit` for the browser suites)
  ...
  tts-box/                # HOME GPU BOX deploy artifacts (NOT droplet/container): TTS upstream systemd user service (Restart=always) + port watchdog (.sh + .timer) + the reverse-tunnel systemd user service (`hosaka-tunnel.service`, ssh `-R 172.17.0.1:8123`, ServerAlive+ExitOnForwardFailure keepalive) + README to keep the Kokoro/Chatterbox TTS upstream alive AND reachable. Installed on Wai's home box, reached only via the SSH reverse tunnel — can't be deployed from the droplet. When the model server dies the tunnel port still accepts+resets (bound port != liveness), so /hosaka shows "TTS server offline". When the HOME BOX reboots the old droplet-side sshd session squats 172.17.0.1:8123 so the respawning tunnel can't rebind (permanent flap) — fixed by SSH keepalive BOTH sides: droplet `/etc/ssh/sshd_config.d/20-hosaka-keepalive.conf` (ClientAliveInterval 30 / CountMax 3, in bootstrap.sh) reaps the dead session in ~90s, home unit's ServerAlive+ExitOnForwardFailure exits cleanly. GatewayPorts drop-in also captured in bootstrap.sh
  ...
  web/                    # static frontend (index.html, fonts, card-dialog.js, images)
    ...
    #   wizard(mtg) watchman(tarot) radar(hosaka) firewall(standalone refresh, nightfall FireWall padlock)
    #   (sentinel/bitman/fiddle.png now unused)
    hosaka-audio.js       # shared HosakaAudio.createPlayer(): owns the AudioContext + iOS unlock dance + the /ws/hosaka WebSocket + playback of streamed 24kHz float32 PCM via scheduled AudioBufferSourceNodes (NOT an AudioWorklet — iOS-safe: context left at hardware rate, AudioBuffer carries 24kHz and resamples on playback; audio unlocked by a silent buffer-source .start() inside the click gesture; iOS Ring/Silent switch defeated by looping silence.wav through an HTMLMediaElement to move the audio session to "playback"). Exposes speak()/unlock()/flush()/setVolume()/elapsed()/audioDuration()/isUnlocked()/gestureUnlocked()/getAnalyser() + per-utterance onStatus/onFirstAudio/onChunk callbacks. `getAnalyser()` returns an AnalyserNode tapping the gain output (a parallel branch with no onward connection, so it reads the signal without touching the audible path; null before the first unlock()) — used by tts.js's /hosaka playback scope. `isUnlocked()` = live `ctx.state==="running"` (RACY — on Windows Chrome/WASAPI it can read non-running right after a gesture even though playback works); `gestureUnlocked()` = sticky "a user-gesture unlock has run at least once" — the correct gate for "may we speak" since speak() itself resumes a suspended context. Used by tts.js (/hosaka — `tts.html` MUST load this script BEFORE tts.js or `HosakaAudio` is undefined), tarot-voice.js (/tarot narration), and exec-voice.js (Exec bubble narration on /rd + /prophecies). The upstream emits only {start}/coarse-PCM-blobs/{end} — NO per-word/fragment timestamps, so callers sync any visual to the measured audio duration, not to word tags
    voice-util.js         # shared voice helper: `VoiceUtil.stripMarkdown(md)` — flattens markdown to spoken prose (drops code fences/spans, bold/italic, headings, link syntax). Used by tarot-voice.js + exec-voice.js so the strip lives in one place (loaded before both)
    exec-voice.js         # Exec bubble narration: `window.execVoice` over a HosakaAudio player, voice `glados` (backend `piper`, gain 0.95 — glados is peak-normalized to ~1.0 by the upstream (measured worst-case peak ~1.013), so 0.95 is about as loud as it goes without clipping; deliberately louder than tts.js's 0.25 RMS-match trim since Exec should be heard). Speaks EVERY Exec turn — assistant replies, monitor comments, timed nudges — AUDIBLE BY DEFAULT (localStorage `exec.voice`, "0" mutes). Mute button `#exec-mute` (✦ glyph) built by exec-bubble.js in the input line, wired via `mountButton()`. speak(md) strips markdown + any `[bracketed]` spans (Wai's own messages + sys notes are never passed in) then fires the player; no-op when muted / not-yet-unlocked. `armUnlock()` one-shot gesture-unlocks a persisted-on voice so nudges narrate after the first interaction (iOS audio rule); opening the panel (a tap) also unlocks. Mirrors tarot-voice's idioms but with no typewriter to pace (fire-and-forget)
    exec-voice-listener.js  # Exec voice on NON-planning protected pages (the link-bubble pages, no chat panel): loads the same `execVoice` glados player, arms the one-shot gesture-unlock, and subscribes to `/api/monitor/stream` to speak Exec's unsolicited turns (monitor comments + timed nudges) on whatever page Wai is on. No assistant replies here (no chat); mute is the same global `exec.voice` flag the planning panel toggles. Loaded by pages.py on every non-planning protected page EXCEPT /tarot (its own reader voice would clash) and /hosaka (the TTS page itself) — those keep the link-bubble, just without the voice scripts
    tts.js tts.css silence.wav  # /hosaka (HOSAKA) TTS page assets: tts.js drives the SPEAK UI (voice list — id only, no description; clone/playback knobs) on top of a HosakaAudio player (shared audio/WS/iOS core now lives in hosaka-audio.js). Carries VOICE_GAIN, a per-voice loudness trim (effective gain = volume knob * trim) so every voice's default lands at ~80% of the clipping limit (output peak ~0.8) at knob 1.0 — gain = 0.8 / each voice's measured worst-case peak (`{nicole:1.35, glados:0.79, charlie:0.97}`; unknown → 1.0). Default voice = `charlie` (chatterbox clone + RVC) when the upstream offers it; default knob values tuned for charlie (intensity 0.85 / guidance 0.75 / randomness 0.15 / speed 1.1, set as the HTML `value=`; `SPEED_CAP` caps charlie's speed slider at 1.1 — RVC clone degrades past it — via `applySpeedCap()` in `reflectBackend`). The CHATTERBOX CLONE KNOBS — relabeled `intensity`/`guidance`/`randomness` (still send `exaggeration`/`cfg_weight`/`temperature`), each with a small-font `.tts-kdesc` summary — are enabled (un-dimmed + clickable; `.off` dims AND `pointer-events:none`) when the selected voice is a chatterbox clone: the upstream's per-voice `cb` flag is authoritative, falling back to `CLONE_BACKENDS` = {chatterbox, rvc} when `cb` is absent; kokoro/piper honor only speed. The SPEAK button doubles as the status indicator (`setBtn`): `Speak` (ready, clickable) -> `generating...` (on {start}, disabled) -> `speaking...` (on {end}, disabled until measured playback finishes) -> back to `Speak`; offline disables it ("offline"). The speed slider is disabled for the whole generating->speaking lifecycle (`setSpeaking` toggles `speaking` + `#tts-speed.disabled`) since speed is a generation param already sent upstream. `setSpeaking` toggles `.tts.busy`, which freezes + dims (opacity 0.45, `pointer-events:none`) every knob block (clone + playback) for the whole generating->speaking lifecycle — the params are locked into the in-flight utterance. `#tts-status` now carries only diagnostic detail (offline reason / error text). A minimalist PLAYBACK SCOPE (`#tts-wave` canvas, between the Speak row and the clone knobs) is ALWAYS PRESENT — `wave.start()` fires once at DOMContentLoaded (`display:block` + column-width/baseline styling — NOT 100vw full-bleed, since `.tts` clips overflow and a viewport-wide scope would be clipped on wide screens) and the rAF loop runs for the page's life. Each frame `wave` (in tts.js) samples `player.getAnalyser()` for one peak amplitude, pushes it onto the right edge, shifts history left; when nothing is playing the analyser reads silence (or is absent before the first unlock), so the trace sits flat on the baseline. Stroke hue read from the canvas `color` in tts.css (`hsl(var(--green-hsl)/0.8)`, palette stays in CSS). Also drives the top-of-page LIVE CONNECTED-USERS count (`mountPresence()` → `#tts-presence`): opens a dedicated presence WS (`/ws/hosaka/presence`), renders the broadcast `{count}` ("N users connected"), reconnects with capped backoff on drop and keeps the last count on screen (never blanks → no layout shift). See routes_tts.py
    tarot-voice.js        # /tarot reader narration: a MUTE button (#voice-btn in #spread-controls, localStorage `tarot.voice`) + a HosakaAudio player speaking the reader's text with voice nicole (kokoro). Narration is AUDIBLE BY DEFAULT (only `tarot.voice == "0"` mutes); the button does nothing but set the player volume to 0 (muted) or 1 (`setOn` + `speak`) — narration still STREAMS while muted, so the typewriter stays audio-paced either way. tarotVoice.ready() = `player.gestureUnlocked()` (a user gesture has unlocked the player; muting only zeroes volume, it does NOT gate narration — NOT the live `isUnlocked()`/ctx.state, which false-negatives on Windows Chrome and silently dropped the voice). The unmute-tap unlocks iOS audio. The auto-triggered opening turn has no gesture (browsers won't play audio pre-gesture), so until unlocked (`wantsDeferredOpening()`) the opening is GENERATED EAGERLY on load — `streamResponse(holdForGesture)` runs the LLM fetch immediately (don't wait for the click to start thinking) but holds the reveal+voice on the `holdForGesture` promise; `armOpeningUnlock(onGesture)` arms a one-shot first gesture that unlocks audio and resolves it, so the click starts the reading with NO LLM round-trip (~tens of ms to first audio). While held, body gets `opening-pending` (CSS blanks the `.reader-cursor` + `#input-bar`) and a faint `.begin-hint` ("tap anywhere to begin") shows; the gesture clears both. Voice off / already unlocked → opening fires + reveals immediately (no hold, no hint). tarot-chat's drainAudio NEVER hangs on a speak failure: a progress watchdog (audio never starts OR stalls mid-utterance >2.5s) and an error/`!ok` check both `bail()` — which marks the controller ended+failed (so the outer wait loop exits) and finishes the text at the guessed pace. The controller carries `error` (set from the `{type:error}` detail / connection failure / `'no audio'` stall); when voice was on but failed, tarot-chat logs an ACTION-font sys note `[ reader voice unavailable: <reason> ]` (not reader prose) so the failure is visible without breaking the reading. The catch block likewise logs `[ error: ... ]` as a sys note instead of overwriting the reader bubble. When there is NO opening turn (returning mid-reading) tarot-chat instead calls `armPersistedUnlock()` — a one-shot `pointerdown`/`keydown` (capture, in-gesture, no setTimeout) that unlocks the player on first interaction so the next reader turn narrates without a re-toggle. tarotVoice.speak(md) strips markdown (via `VoiceUtil.stripMarkdown`, shared with exec-voice), fires TTS, returns a controller {elapsed(),duration(),ended,ok} the typewriter polls. NOTE: /ws/hosaka accepts EITHER a full `session` OR a `guest_session` cookie, so the reader voice works for guests on /tarot too; only a no/invalid-cookie WS closes 1008 → tarot-chat logs `[ reader voice unavailable: connection failed ]` and falls back to the silent guessed-pace typewriter
    tarot-music.js        # /tarot ambient background music: a looping <audio> element streamed lazily (`preload="none"` -> nothing fetched until play(), so ZERO page-load cost; the browser then streams progressively via HTTP range). 15% bed level, no ducking (voice sits over it). VOLUME IS BAKED INTO THE FILE (re-encoded at 0.15 gain) because on iOS HTMLMediaElement.volume is a no-op AND routing through a WebAudio MediaElementSource SILENCES the element (don't do it — an earlier gain-node attempt killed iPhone playback and also fought the voice player's AudioContext, breaking the opening narration); a plain <audio> with the level baked in is the only thing that plays + stays quiet on both desktop and iOS. el.volume is used ONLY for the desktop fade-in (0->1, 4s, requestAnimationFrame); iOS ignores it and starts at the file's level. Start (and fade) happen on the FIRST user tap (`arm()` one-shot pointerdown/keydown; autoplay is gesture-gated anyway and the fade must begin on the tap, not on load). On first start it also seeks a RANDOM point (`loadedmetadata` -> `currentTime = Math.random()*duration`, 206 range). iOS Ring/Silent-switch defeat: startPlayback() loops a silent `/silence.wav` HTMLMediaElement in-gesture to move the page audio session to the "playback" category (same trick as hosaka-audio.js, NO AudioContext) — without it a plain <audio> is muted by the hardware switch. Stop/play toggle button (#music-btn, ♫ glyph) appended BELOW #reset-btn in #spread-controls (state in localStorage `tarot.music`, default on). Track = web/tarot-ambient.m4a (audio/mp4, ~58MB, gitignored — lives on the server, volume-mounted; MUST be a PROGRESSIVE faststart mp4 at the target volume — re-encode with `ffmpeg -i in -af volume=0.15 -c:a aac -b:a 128k -movflags +faststart -vn out`; a fragmented/DASH mp4 storms range requests on seek). tarot.html loads it after hosaka-audio.js
  ...
  api/
    ...
    graph_scrub.py        # serve-time scrubbing of graphify's /graph HTML (imported by graph_page): _redact_graph_nodes() blanks leaky node summaries; _drop_graph_book_nodes() cuts the api/tarot/book/ Pollack reference nodes; _drop_graph_moltbook_nodes() cuts the moltbook heartbeat node(s) by id/label/source substring — both via shared _prune_graph_nodes() (drops the nodes + their edges/legend rows/hyperedges, the narrative "shaded region" clusters off the book, e.g. "...gathered into the Chariot's ego"). _name_graph_communities() renames generic "Community N" legend labels + node community_name to a Title-Cased name from each community's dominant source file (run after the drops). _size_graph_by_loc() rescales node size to track line count (file node = whole-file lines, symbol = span to next def) from graph.json's source_location start lines (no file reads — sibling graph.json, sqrt-compressed into ~10..40). Per-line anchored array match (^NAME = [...];$, optional const/var/let) — a non-greedy [.*?] would truncate at a `];` inside a node title. Runs per request so edits survive /graphify rebuilds
    ...
    routes_tts.py         # /hosaka TTS page + same-origin reverse-proxy to a home GPU box (Kokoro/Chatterbox) reached over an SSH tunnel on the Docker bridge gateway (TTS_UPSTREAM, default 172.17.0.1:8123). Serves /hosaka (GUEST-or-full auth — `guest_protected`; nav renders guest-tier for non-admins), GET /api/hosaka/voices (guest; proxies the upstream voices list), GET /api/hosaka/health (guest; probes the upstream — `{ok}`/503; a bound tunnel port isn't liveness, only a real response is, so /hosaka polls this to show "TTS server offline" before SPEAK), and the WS /ws/hosaka (public route, but rejects unless a `session` OR `guest_session` cookie matches — so guests get the /tarot reader voice too — proxies the audio stream both ways), plus the WS /ws/hosaka/presence (same cookie gate; live connected-users count — every open /hosaka tab holds one socket, the module-level `_presence` set is the source of truth, `_broadcast_presence()` pushes `{count}` to all on each join/leave). Cookie auth rides the WS handshake (basic-auth doesn't, on mobile). Needs the `websockets` dep.
    ...
    templates/            # volume-mounted → live on save
      ...
      tts.html            # /hosaka — HOSAKA SPEAK UI (no title; the page never scrolls — `.tts` is `overflow:hidden` and the textarea `flex:1 1 0` shrinks/grows so everything fits one screen; Speak button + voice select share one `.tts-row`, clone/playback knobs below). All Iosevka. Assets web/tts.{js,css}
...
- `guest_session` cookie (set via `POST /guest`, requires `GUEST_KEY`) — only `/mtg`, `/tarot`, `/nightfall`, `/hosaka`. Login form at `GET /guest`. `GET /guest-login` is a 302 alias to `/guest` (bookmark compat).
