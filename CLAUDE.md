...
exec-fn/
  ...
  web/                    # static frontend (index.html, fonts, card-dialog.js, images)
    ...
    tarot-voice.js        # /tarot reader narration: a voice toggle (#voice-btn in #spread-controls, localStorage `tarot.voice`, default OFF) + a HosakaAudio player speaking the reader's text with voice af_nicole (kokoro). tarotVoice.ready() = on AND audio unlocked (a gesture ran — the toggle-tap unlocks iOS audio; the auto-triggered opening turn has none, so it stays silent until first interaction). tarotVoice.speak(md) strips markdown, fires TTS, returns a controller {elapsed(),duration(),ended,ok} the typewriter polls. NOTE: /ws/hosaka requires the full `session` cookie, so audio only works for logged-in Wai — guests (guest_session) fall back to the silent guessed-pace typewriter
