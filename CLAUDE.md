...
**Pulling on the droplet — do NOT `git stash -u`.** The compose file bind-mounts **untracked** dirs (`./nightfall-incident` → `/app/nightfall`, `./graphify-out`). `git stash -u` (or anything that deletes+recreates those dirs) STALES the running container's bind mount — the container then can't see files that exist on the host, and the next `--reload` crashes at import (`routes_nightfall` reads `nightfall-incident/wai-head.js` at module load → **site down, 502**). To pull past a dirty `graphify-out`, use `git checkout -- graphify-out` (it's regenerated by `/graphify`), never `stash -u`. If a mount goes stale anyway: `docker compose up -d --force-recreate api`.
