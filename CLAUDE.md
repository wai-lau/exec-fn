...
| `/debug` | Profile notes + activity log viewer + saved tarot readings. Styled as **"dark recruiter"** (`debug.css`): the same document-card language as `/recruiter`'s dark mode — a centered dark card, terminal-green ink, `.cv h2`-style section headers with bottom rules, chips for tags. Page-local `--dbg-*` tokens mirror recruiter's `html.cv-dark` `--cv-*` set by hand (recruiter = source of truth; a shared doc-theme extraction is the eventual home). |
