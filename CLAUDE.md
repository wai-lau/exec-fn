...
exec-fn/
  ...
  web/                    # static frontend (index.html, fonts, card-dialog.js, images)
    ...
    emet.css              # /emet skin: fullscreen graph + the two collapsible panels (physics strip / node-info drawer) + toggles + vis-config dark restyle. Injected by emet_page
    emet-overlay.js       # /emet behavior: builds physics menu (vis configurator into a bottom strip) + node-info drawer toggles, mirroring /graph's graph-overlay. Reads window.net from the emet.html renderer
...
| `/emet` | **Protected** vis-network knowledge graph (Wai's personal graph). `emet_page()` in routes_views.py serves the `templates/emet.html` renderer and injects the graph DATA inline as `window.EMET_GRAPH` from `templates/emet-graph.json` (`{meta,nodes,edges}` — `<` escaped to `<`). **Both emet.html + emet-graph.json are gitignored sensitive personal data — never commit** (see memory). Same UI shell as /graph: chrome.css + cyber-fx bg + bottom nav + content-hash ETag/`no-cache` cache-bust. `web/emet-overlay.js` + `web/emet.css` add /graph-style collapsible panels: a **physics** menu (vis configurator, bottom strip, collapses down) and a **node-info** drawer (`#side` observations, right, collapses right, opens on node click); leaf labels kept readable (font floor 15px). Data lives in `api/templates/` (not the public `/app/static` mount) so it stays auth-gated. Nav label = `אמת` (emet, "truth"), icon = `golem-stone.png` (nightfall GolemStone sprite), next to graph. |
