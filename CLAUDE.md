...
- **Everything in hq has a plan**: each tick, actionable hq cards missing a graph get a silent decompose (`_build_graph` — no nudge sent). The breakdown is visible/editable in the card dialog (`web/card-graph.js` SVG DAG, shown when `card.nudge.graph.nodes` non-empty; edits persist on dialog save, `active_node` recomputed client-side with the same first-open rule).
