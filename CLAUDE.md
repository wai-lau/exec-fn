...
| `/emet` | **Protected** static page. `templates/emet.html` (gitignored — edit on-server freely) served via `emet_page()` in routes_views.py, which injects the same UI as /graph: chrome.css + cyber-fx bg (`.cyber-bg`/`.cyber-scan`) + bottom nav, plus a content-hash ETag + `Cache-Control: no-cache` so edits cache-bust (the path has no extension, so the no-cache middleware skips it). The graph-overlay css/js are NOT applied (vis-network-specific). Lives in `api/templates/` (not the public `/app/static` mount) so it stays auth-gated. Nav label = `אמת` (emet, "truth"), icon = `golem-stone.png` (nightfall GolemStone sprite), next to graph. |
