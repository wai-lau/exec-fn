...
| `/debug` | Profile notes + activity log viewer + saved tarot readings. Styled via the shared **document theme** — chrome.css defines `--doc-*` tokens built ENTIRELY from the palette (`--doc-ink` = `hsl(var(--green-hsl)/0.8)`, surfaces = green mixed into `--bg-hsl`, links = cyan, `--doc-pink` = `--pink-hsl`; no new colours, every alpha within green's 4-step budget) + reusable `.doc-card` / `.doc-h2` / `.doc-chip` classes (centered dark card, green glow, phosphor ink, ruled uppercase section headers, tag chips). The reading/dashboard pages (`/debug`, `/emet`, `/color`, `/mtg`, `/tarot` chat, `/security`) apply those classes in their template + style only page-specific content on the `--doc-*` tokens; the dense boards (`/rd`, `/hq`) stay as-is. (`/recruiter` keeps its own `--cv-*` light/dark palette — a separate deliberate departure.) |
| `/security` | **Public** (no auth). ONE long page (no tabs), framed as the automated bot/scanner traffic any public server sees — deliberately NOT a script-kiddie attack-count wall. `security.py`'s `render_security()` builds inline-SVG charts from `data/security.json`, wrapped in the standard shell via `_render_page("security", ..., guest=…)` (non-owners get the guest nav). Sections top→bottom: an intro line ("humans behind it number in the single digits"), the **origin map** (hero, first; `dotmap(zoom=1.2)` = a 20%-zoomed equirectangular world, dot size=volume, red=SSH-login bots / cyan=web scanners; the projection crops empty ocean/poles centred on 0,0), **Scanner traffic by country** \| **Networks hosting the bots** (datacenter ASNs), and **What the scanners hunt for** (top 404 probe paths — `/.env`, `/.git/config`, `wp-admin`, …, none of which exist here). **No owner-identifying data is rendered** (no owner-IP `◄ you` marker, no raw attacker-IP table, no usernames/bans) — the page is public. DELIBERATELY drops the user-agent `kind` "human vs bot" split: `kind` is UA-based, bots spoof browser UAs, so its "human" bucket (tens of thousands) inverts the whole point. The JSON is produced OUT-OF-BAND by a **host** cron (`scripts/security/refresh.py`, runs as root OUTSIDE the container) that parses `/var/log/{auth,nginx/access,fail2ban}.log*`, geolocates the top IPs via ip-api.com (cached in `data/geo_cache.json`), and atomically writes `data/security.json` (gitignored) — keeping host log access out of the internet-facing app. `SECURITY_OWNER_IP` still feeds the cron (owner-hit accounting) but is no longer surfaced in the page. Nav icon = `sentinel.png`; default favicon kept. Missing/empty JSON → "not generated yet" placeholder. Styled on the shared **document theme** (`.doc-card` wrapper + `--doc-*` tokens in `security.py`'s inline `_CSS`); the SVG chart DATA colours stay distinct for the viz. |
...
| GET | `/api/color/usage` | Public. `{counts, alphas, alpha_counts, sites}` — `var(--X)` reference counts + actually-used alphas per `-hsl` token + parallel per-alpha counts + `sites` (`{token: {α-string: {site-label: n}}}`, site = nearest CSS selector else filename), across templates + web assets + **all `api/*.py`** (several ship inline CSS — `security.py` alone references ~56 tokens — that would otherwise misflag used tokens as unused on /color; chrome.css `:root` block excluded; bare `hsl(var(--X-hsl))` = α 1). Feeds the alpha columns, per-opacity ×N, per-column usage-site lists, and the Scale-section counts/flags on `/color` |
