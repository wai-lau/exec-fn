...
| `/color` | **Public** (no auth — palette only, no data). Read-only moodboard: one little table per color (hue-ordered, wrapping). Title (friendly `[Name]`) on top; table columns are the color's variations — card colors show chore/task/project/titan, non-card `-hsl` colors their used alpha steps; rows are swatch / opacity / count / effects; usage description under the table. Tokens with the same H S L merge into one (max 4 variations); a non-card token's alpha usages map onto the nearest card size (`SIZE_ALPHA` = chore .15 / task .25 / project .8 / titan 1) and their `alpha_counts` land in the count row. Edit colors in chrome.css; this page just watches. Admin cookie → full nav, else guest nav |
