...
exec-fn/
  ...
  api/
    ...
    routes_tts.py         # /hosaka TTS page + WS reverse-proxy (/ws/hosaka) + voices proxy (/api/hosaka/voices) to the home GPU server over the SSH tunnel
...
| `/hosaka` | **Protected** text-to-speech page. `routes_tts.py` serves `templates/tts.html` (voice dropdown w/ descriptions + param sliders + `web/pcm-player.js` AudioWorklet, styled via `web/tts.css`) and reverse-proxies its WebSocket `/ws/hosaka` + voice list `/api/hosaka/voices` to a home GPU TTS server (Kokoro presets + Chatterbox cloning) reached at `172.17.0.1:8123` (docker bridge → host SSH tunnel; override `TTS_UPSTREAM`). Same-origin so the session cookie rides the WS handshake. Not in the nav — reach by direct URL. |
...
| WS | `/ws/hosaka` | TTS WebSocket reverse-proxy to the home GPU server (one JSON request per utterance in; `{"type":"start"}` / binary PCM / `{"type":"end"}` out). Session cookie checked in-handler before accept. |
| GET | `/api/hosaka/voices` | Proxy the upstream TTS voice list (`{id, backend, source, description}`). |
