...
| Term | Meaning |
|------|---------|
| R&D | Main kanban — all cards (`rd.json`) |
...
- `is_book`: true = ongoing read — shown in books bar on hq, hidden from rd/hq columns in kanban, never scheduled/decomposed (checkbox in card dialog, like `is_reminder`)
