...
exec-fn/
  ...
  web/                    # static frontend (index.html, fonts, card-dialog.js, images)
    ...
    tts.js tts.css silence.wav  # /hosaka (HOSAKA) TTS page assets: tts.js drives the SPEAK UI (voice list — id only, no description; clone/playback knobs) on top of a HosakaAudio player (shared audio/WS/iOS core now lives in hosaka-audio.js). Carries VOICE_GAIN, a per-voice loudness trim (effective gain = volume knob * trim) so every voice plays at ~the same level by default — derived from each voice's measured RMS of a fixed sentence normalized to ~0.05 (glados clipped; currently `{nicole:0.98, glados:0.25}`, charlie/unknown → 1.0). Default voice = `charlie` (chatterbox clone + RVC) when the upstream offers it; default knob values tuned for charlie (intensity 0.85 / guidance 0.75 / randomness 0.15 / speed 1.1, set as the HTML `value=`; `SPEED_CAP` caps charlie's speed slider at 1.1 — RVC clone degrades past it — via `applySpeedCap()` in `reflectBackend`). The CHATTERBOX CLONE KNOBS — relabeled `intensity`/`guidance`/`randomness` (still send `exaggeration`/`cfg_weight`/`temperature`), each with a small-font `.tts-kdesc` summary — are enabled (un-dimmed + clickable; `.off` dims AND `pointer-events:none`) when the selected voice is a chatterbox clone: the upstream's per-voice `cb` flag is authoritative, falling back to `CLONE_BACKENDS` = {chatterbox, rvc} when `cb` is absent; kokoro/piper honor only speed. The SPEAK button doubles as the status indicator (`setBtn`): `Speak` (ready, clickable) -> `generating...` (on {start}, disabled) -> `speaking...` (on {end}, disabled until measured playback finishes) -> back to `Speak`; offline disables it ("offline"). The speed slider is disabled for the whole generating->speaking lifecycle (`setSpeaking` toggles `speaking` + `#tts-speed.disabled`) since speed is a generation param already sent upstream. `setSpeaking` stops the `wave` scope (it STARTS on first audio, not at click — see the SCOPE note below) and toggles `.tts.busy`, which freezes + dims (opacity 0.45, `pointer-events:none`) every knob block (clone + playback) for the whole generating->speaking lifecycle — the params are locked into the in-flight utterance. `#tts-status` now carries only diagnostic detail (offline reason / error text). A minimalist PLAYBACK SCOPE (`#tts-wave` canvas, between the Speak row and the clone knobs) is HIDDEN (`display:none`) until audio actually plays — `wave.start()` fires from the player's `onFirstAudio` (NOT at click, so it never sits blank through `generating...`, and it frees that vertical space until needed), adding `.live` to reveal it (display:block + column-width/baseline styling — NOT 100vw full-bleed, since `.tts` clips overflow and a viewport-wide scope would be clipped on wide screens). While live it scrolls a waveform leftward — `wave` (in tts.js) samples `player.getAnalyser()` for one peak amplitude per animation frame, pushes it onto the right edge, shifts history left; stroke hue read from the canvas `color` in tts.css (`hsl(var(--green-hsl)/0.8)`, palette stays in CSS). On stop it does NOT clear immediately — `drainLeft` keeps the loop running while appending silence so the captured wave keeps marching left off the edge (~one canvas width of frames), then drops `.live` and hides the scope again. See routes_tts.py
...
| `/hosaka` | **Guest-or-full** TTS page (`guest_protected` — guests welcome). Default voice `charlie`. SPEAK UI streaming audio from a home GPU box (Kokoro/Chatterbox) via a same-origin WS reverse-proxy (`routes_tts.py`); browser only ever talks same-origin so the session/guest cookie carries auth on the WS handshake. |
