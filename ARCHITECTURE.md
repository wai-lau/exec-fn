...
**Tapping a node fires a cascade seeded on it** (`graphPulse.seed(id)`, wired to vis's `click`; graph.html's own handler still opens the node panel, vis takes both listeners). It is the ordinary iteration with its seed chosen instead of drawn — same burst, same stagger, same outward walk — because what is interesting about a node is what it reaches, and the cascade is the thing that draws what it reaches. It works on a phone even though ambient seeding does not: `init` now sets `auto` from `(pointer: coarse)` and only the SEEDING clock is gated, never the rAF loop. An idle frame paints nothing, so nothing under the glass changes and the backdrop-filter readback never re-fires — what a phone cannot afford is continuous lighting, not the loop. Verified in WebKit by reading lit pixels off the canvas: desktop ambient peak 261 / after a node click 295; phone (`has_touch`, coarse) ambient **0** / after a node tap 61.
