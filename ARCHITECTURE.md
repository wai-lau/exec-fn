...
**The firing canvas moved ABOVE the CRT stack** (2026-09-22), reversing the deliberate choice to keep the glow behind the same glass as the graph. §10's rule is that a `backdrop-filter` is a full-viewport readback with no partial invalidation, ruinous over anything animated — and /graph had the page's only animated layer sitting under two of them, so it was the standing violation of its own documented rule. The droplet measurement (2fps under the stack against 18 with it hidden, our own canvas work 0ms either way) was filed as a headless-software-compositing curiosity and dismissed for real hardware. It was not a curiosity: on a 430x932 phone at DPR 3 the display stopped updating entirely, so the loading cover sat frozen mid-fill at `drawing` while JS ran on behind it and the 3s reveal failsafe fired into a screen that never repainted. A screenshot of that state is what identified it — the bar stuck at `DRAWN` with `data 0.1s · build 0.1s · place 0.0s` underneath, every measured phase fast and the page still apparently dead, which rules out the work and leaves the compositing. At `--z-modal` the canvas paints over the filters, nothing beneath them changes, and the readback never re-fires. It ties with the fx layers on z and wins on DOM order — load-bearing, not incidental: the fx divs are in the served HTML and the canvas is created by JS at init, so it is always later. Verified at 430x932 DPR 3: pulse z 9990 / DOM index 18, blur and crt 9990 / 6 and 7, nav 9999, 2,722 nodes, no page errors.

**The lattice is shaped to the viewport, and the opening view fills it.** graphify's cloud is roughly square; the window rarely is. At 430x932 the camera opened on cover, filled the height and ran **2.08x the width** — a phone saw the middle strip of the graph and had to drag for the rest. `snapToGrid` now rescales every node into a box of the viewport's own aspect before it snaps, and the box **keeps the original area** (`boxW*boxH == b.w*b.h`), so `cell` comes out unchanged and the lattice is exactly as dense as before. The cells stay SQUARE: the reshape is in which cell a node lands in, never in the spacing, or the nodes themselves would read as stretched. Measured: viewport aspect 0.50 -> cloud 0.50 at 430x932, 1.72 -> 1.73 at 1280x800.
...
Stabilising this graph is ~270 forceAtlas2 iterations over 2722 nodes. On a desktop that is a few seconds under the loading cover; **on a phone it measured around thirty seconds, on every single visit**, for a layout that comes out the same every time.
