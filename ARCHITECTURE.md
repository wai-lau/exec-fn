...
**Tint, 2026-09-21.** The phosphor read heavy on every page, so `.cyber-bg`'s stripe went `0.45 → 0.25` and `.cyber-crt`'s contrast `1.5 → 1.25`. Two things worth keeping: the alpha moved one step DOWN the snap scale rather than to a freehand value (an off-scale alpha is lint-rejected, and rescaling these gradients is exactly what crt-dim.css was told not to do), and the contrast had to come with it — that filter multiplies whatever green survives, so halving the stripe alone left the punch doing half the tinting. The two `0.06` glows are already the lowest non-zero step on the scale.
